...
mon_canton <- surfaces_ra1852 %>% 
  filter(canton == "BETHUNE") %>% ### Changer ici avec votre canton
  pivot_longer(-c(code, canton, departement)) %>% 
  select(-c(code, canton, departement)) %>% 
  print()
```

Alors quelle est le type de terre labourable la plus étendue ? Chez nous ce sont les céréales.   Moi aussi
Avez-vous un peu de vigne ? Non pas du tout
...
```{r}
Controle_superficie <- mon_canton %>% 
  pivot_wider() %>% 
  mutate(TOT_SUP = total_des_terres_labourables - cereales - racines_et_legumes_divers - cultures_diverses - prairies_artificielles - jacheres)
```
...
```
curl 'https://geo.api.gouv.fr/communes?nom=Harnes&codeDepartement=62&fields=centre&boost=population&limit=1'
...
Les coordonnées renvoyées dans le terminal sont inversées !!!!
...
url_completed <- sprintf(url, "BAR-SUR-AUBE", "10")
...
url_api2 = "https://geo.api.gouv.fr/communes?nom=%s&codeDepartement=%s&fields=centre&boost=population&limit=1"
...
surfaces_ra1852_latlong <- surfaces_ra1852_coordonnees_centre_corrigees %>% 
  mutate(latitude=str_split_fixed(centre,",",2)[,1],
         longitude=str_split_fixed(centre,",",2)[,2]) %>% 
  mutate(latitude=substr(latitude,3,9)) %>% 
  mutate(longitude=substr(longitude,1,nchar(longitude)-1))
